...
class BDCGAN(object):
    ...
    def __init__(self, x_dim, z_dim, dataset_size, batch_size=64, gf_dim=64, df_dim=64, 
        ...
        self.K = 1 # fake and real classes
        self.x_dim = x_dim
        self.z_dim = z_dim
    ...
    def _get_optimizer(self, lr):
        if self.optimizer == 'adam':
            return tf.compat.v1.train.AdamOptimizer(learning_rate=lr, beta1=0.5)
        elif self.optimizer == 'sgd':
            return tf.compat.v1.train.MomentumOptimizer(learning_rate=lr, momentum=0.5)
        else:
    ...
    def initialize_wgts(self, scope_str):
        ...
        param_list = []
        with tf.compat.v1.variable_scope(scope_str) as scope:
            for zi in xrange(numz):
                for m in xrange(self.num_mcmc):
                    wgts_ = AttributeDict()
                    for name, shape in weight_dims.items():
                        wgts_[name] = tf.compat.v1.get_variable("%s_%04d_%04d" % (name, zi, m),
    ...
    def build_bgan_graph(self):
        ...
        self.z = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.z_dim, self.num_gen], name='z')
        self.z_sampler = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.z_dim], name='z_sampler')
        ...
        # prep optimizer args
        self.d_learning_rate = tf.compat.v1.placeholder(tf.float32, shape=[])
        ...
        for di, disc_params in enumerate(self.disc_param_list):
            ...
            for gi, gen_params in enumerate(self.gen_param_list):
                d_probs_, d_logits_, _ = self.discriminator(self.generator(self.z[:, :, gi % self.num_gen], gen_params,reuse                                                                 =False),self.K, disc_params,reuse=True)
            ...
            d_opt_adam = tf.compat.v1.train.AdamOptimizer(learning_rate=self.d_learning_rate, beta1=0.5)
            self.d_optims_adam.append(d_opt_adam.minimize(d_loss, var_list=self.d_vars[di]))

        ### build generative losses and optimizers
        self.g_learning_rate = tf.compat.v1.placeholder(tf.float32, shape=[])
        self.g_vars = []
        ...
        for gi, gen_params in enumerate(self.gen_param_list):
            ...
            for disc_params in self.disc_param_list:
                d_probs_, d_logits_, d_features_fake = self.discriminator(self.generator(self.z[:, :, gi % self.num_gen],
                                                                                         gen_params,reuse=False),
                                                                          self.K, disc_params,reuse=True)
                _, _, d_features_real = self.discriminator(self.inputs, self.K, disc_params,reuse=False)
            ...
            g_opt_adam = tf.compat.v1.train.AdamOptimizer(learning_rate=self.g_learning_rate, beta1=0.5)
            self.g_optims_adam.append(g_opt_adam.minimize(g_loss, var_list=self.g_vars[gi]))

        ### build samplers
        self.gen_samplers = []
        for gi, gen_params in enumerate(self.gen_param_list):
            self.gen_samplers.append(self.generator(self.z_sampler, gen_params,reuse=True))
    ...
    def discriminator(self, image, K, disc_params, train=True, reuse=False):
        ...
        with tf.compat.v1.variable_scope("discriminator", reuse=reuse) as scope:
            ...
            h_end = tf.nn.relu(linear(tf.reshape(h, [self.batch_size, -1]),
            ...
            return tf.nn.sigmoid(h_out), h_out, [h_end]
            

    def generator(self, z, gen_params, reuse=False):
    ...
    def gen_noise(self, gen_params): 
        with tf.compat.v1.variable_scope("generator") as scope:
            ...
            for name, var in gen_params.items():
                noise_ = tf.compat.v1.distributions.Normal(loc=0., scale=self.noise_std*tf.ones(var.get_shape()))
